...
- **FileViewerView** — read-only viewer with line numbers + Highlightr syntax highlighting (reuses the shared `SyntaxHighlighter.shared.highlight(line:language:isDark:)` from the diff pane). Vertical-only `ScrollView` so each row's `.frame(maxWidth: .infinity)` actually expands to the column width and wrapped lines reflow when the splitter is dragged — horizontal-scroll variants collapse rows to intrinsic size. Per-line gutter width is computed from `String(lines.count).count * 7 + 6` so a 20-line file doesn't get a 4-digit-wide gutter. Owns its own `@FocusState contentFocused` + `@State focusedLine` (0-indexed, resets to 0 on every new file). Single `.onKeyPress(keys:)` for `.upArrow` / `.downArrow` to move `focusedLine` with `ScrollViewReader` proxy auto-scrolling to `.center`. `cachedLines` + `cachedLinesKey` (`url|byteCount`) memoize the line split so arrow keypresses don't re-split a 25K-line file as a side-effect of body re-eval; the cache is refreshed in `.onChange(of: viewModel.selectedFileContent)`/`selectedFileURL` and falls back to an inline split on the first body eval after a file switch to avoid flashing stale rows from the previous file. Grabs keyboard focus whenever `viewModel.focusGeneration` bumps and `focusTarget == .viewer` (i.e. user pressed Enter on a file in the tree). The focused-line tint only shows while the viewer actually has keyboard focus, so the active pane is obvious at a glance.
...
To make ⇧⌘T available for the shell pane toggle, "New Todo" moved from ⇧⌘T to ⌥⌘T in `OdinApp.commands`. ⇧⌘N (New Note) and ⌘N (New Claude Session) are unchanged. `ContentView` now appends the ⌃1…⌃4 tab-switch hints inline in the macOS tab titles (`"Claude  ⌃1"`, etc.) since `Tab` doesn't render menu shortcuts. The sidebar shortcut-hint footer in `ClaudeSessionListView` lists New session / Project pane / Diff pane / Review pane / Terminal pane via a small helper.
